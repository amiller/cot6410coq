...
(** **** Exercise: 2 stars, optional *)
(** The [replace] tactic allows you specify a particular subterm to
   ...
   subgoal. This is often when a plain [rewrite] acts on the wrong
   part of the goal.  Use the [replace] tactic to do a proof of
   [plus_swap'], just like [plus_swap] but without needing [assert (n
   + m = m + n)]. *)
Theorem plus_swap' : forall n m p : nat, 
  n + (m + p) = m + (n + p).
Proof.
